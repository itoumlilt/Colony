...
// Represents a bucket in the colony database.
// Offers a high-level interface to issue read and write operations on objects in the bucket.
type Bucket struct {
	Bucket []byte
}

// A transaction object offers low-level mechanisms to send protocol-buffer messages to colony in the context of
// a highly-available transaction.
// Typical representatives are interactive transactions handled by colony and static transactions handled on the client side.
...
// A transaction handled by colony on the server side.
...
// and cleans up the server side.
// WARNING: May not be supported by the current version of colony

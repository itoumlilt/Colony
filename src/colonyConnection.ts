...
export class colonyConnection {
	...
	private readMessagesFromBuffer() {
		...
		while (buffer.remaining() >= 4) {
			...
			if (code != 0) {
				...
				let errorProto: colonyPB.ApbErrorRespMessage = MessageCodes.colonyPb.ApbErrorResp.decode(slice.toBuffer(true));
	...
	private handleError(errorCode: number, errorMsg: string) {
		let request = this.requests.shift();
		if (request) {
			this.reject(request, new Error(`colony-PB Error code ${errorCode}:\n${errorMsg}`))
		} else {
